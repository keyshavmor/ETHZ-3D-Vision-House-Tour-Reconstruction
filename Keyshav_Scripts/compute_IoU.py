...
file2 = 'points3D_backprojection_labelled.txt'  # Replace with the actual file name
...
with open(file2, 'r') as f:
    ...
    for line in lines:
        ...
        if label in line_data[6:8]:  # Check if the label is present in either the 7th or 8th position
            num_points += 1
            num_points_file2 += 1
            xyz2 = np.array(line_data[:3], dtype=np.float128)  # X, Y, Z coordinates
            rgb2 = np.array(line_data[3:6], dtype=np.int64)  # RGB values
