...
class OVSegVisualizer(Visualizer):
    ...
    def draw_sem_seg(self, sem_seg, area_threshold=None, alpha=0.8):
        ...
        for label in filter(lambda l: l < len(class_names), labels):
            ...
            print(text)
            # Iterate through the label_masks dictionary
            for color, label in label_masks.items():
                if label == text:
                    #Convert the RGB values to a colour mask by dividing by 255
                    mask_color = tuple(c / 255 for c in color)
                    break
                else:
                    # Handle the case when the incoming text does not match any label
                    mask_color = None
