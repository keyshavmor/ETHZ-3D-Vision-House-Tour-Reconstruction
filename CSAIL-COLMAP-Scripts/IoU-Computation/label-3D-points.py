...
filename = 'points3D_colmap_backprojection.txt'  # Replace 'your_file.txt' with the actual file name
...
for line in lines[3:]:  # Ignore the first three lines
    ...
    if len(values) >= 8 and values[7] in labels.values():
        ...
        if len(values) >= 8:
            values[7] = best_label if best_similarity > 0.80 else "None"  # Assign label if similarity is above threshold
        else:
            values.append(best_label if best_similarity > 0.80 else "None")
...
output_filename = 'points3D_colmap_backprojection_labelled.txt'  # Replace 'processed_file.txt' with the desired output file name
