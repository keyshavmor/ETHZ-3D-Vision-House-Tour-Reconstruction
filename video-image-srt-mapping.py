...
# Parse subtitle file
sub_file = 'final-apartment-tour-final.srt'
...
for i, subtitle in enumerate(subs):
    ...
    if len(lines) >= 3:
        sub_idx = lines[0]
        sub_time = lines[1].split(' --> ')
        start_time = pd.Timestamp(sub_time[0]).time()
        end_time = pd.Timestamp(sub_time[1]).time()
        sub_title = lines[2]
        sub_meta = {
            'idx': sub_idx,
...
video_file = 'apartment-final-tour.mp4'
...
df.to_excel('frame_srt_keyword_label_map.xlsx', index=False)
